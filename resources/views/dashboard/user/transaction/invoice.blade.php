        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12 mb-0"><a href="{{ route('user.dashboard.home') }}">Home</a> <span
                        ...
                        class="text-black">Transaction/{{ $transaction->reference_no }}</strong>
                </div>
            </div>
        </div>
    </div>
    <div class="site-section">
        <div class="container">
            <div class="col-md-12">
                <div class="invoice">
                    @php
                        $orderItem = $transaction->order->orderItems->first(); // Get the first order item
                        $price = $orderItem ? $orderItem->price : 'N/A'; // Retrieve the price or set to 'N/A' if not available
                    @endphp
                    ...
                    <div class="invoice-header">
                        ...
                        <div class="invoice-date">
                            <small>Reciept / Generated</small>
                            <div class="date text-inverse m-t-5">{{ $transaction->created_at->format('F d, Y') }}</div>
                            <div class="invoice-detail">
                                {{ $transaction->reference_no }}<br>
                                {{ $transaction->type }}
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="table table-invoice">
                                ...
                                <tbody>
                                    @foreach ($transaction->order as $order)
                                        <tr>
                                            <td>
                                                <span class="text-inverse">{{ $order->product_id->description }}</span><br>
                                                <!-- You can display other order details here -->
                                            </td>
                                            <td class="text-center">
                                                ${{ $order->orderItems->price }}<br>
                                                <!-- Display other order item details here -->
                                            </td>
                                            <td class="text-right">
                                                {{ $order->orderItems->quantity * $order->orderItems->price }}
                                            </td>
                                        </tr>
                                    @endforeach
