    ...
    <div class="main-content">
        <div class="section__content section__content--p30">
            ...
            <div class="container-fluid au-card mb-2 col-xl-8 col-lg-8 col-md-12 col-sm-12">
                <form action="{{ route('admin.dashboard.brand.update', $brand->id) }}" enctype="multipart/form-data"
                    method="post">
                    @csrf @method('PUT')
                        ...
                        <div class="col-xl-8 col-lg-8 col-md-12 col-sm-12">
                            <div class="form-group">
                                ...
                                <input id="cc-payment"type="file" name="logo"
                                    ...
                                    placeholder="upload logo" value="{{old('logo') ?? $brand->logo}}">
                                @error('logo')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="form-group">
                                ...
                                <select id="status" name="status"
                                    ...
                                    @foreach ($statusOptions as $status)
                                        <option value="{{ $status }}" {{old('status', $brand->status) == $status ? 'selected' : ''}}>
                                            {{ $status }}</option>
                                    @endforeach
                                </select>
                                @error('status')
